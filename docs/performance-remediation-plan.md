...
- [x] **1.1 — Add the two status-scoped partial indexes on `reconciliation_exception` [S]**
  Landed in `supabase/migrations/20260904000001_review_queue_perf_rewrite.sql`. **Applied to the live DB 2026-09-05** via `npx supabase db push` (user-run, outside any session working this plan).
...
- [ ] **1.6 — Verify the whole phase on `/review` [S]**
  ...
  **Still not done.** No live Supabase connection has ever been available in the sessions that worked this plan (MCP Supabase servers are unauthenticated here) — `EXPLAIN ANALYZE` and before/after timings need a human with `pbi`/`supabase` access. `npx tsc --noEmit`, `next lint`, and `vitest run` all pass against the application-code changes (1.4's `queueCountQuery`), but the SQL itself remains unverified against a real plan.
  **Update 2026-09-05:** all migrations for this plan, including 1.1–1.5, are now confirmed applied to the live DB (`npx supabase migration list` shows every local migration through `20260905000003` with a matching remote entry, applied by the user). What's left for this item is purely the manual "open a bill, step J/K, watch it feel faster, run EXPLAIN ANALYZE" pass — no longer blocked on applying anything.
...
- [~] **3.5 — Confirm the Vercel plan tier honours `maxDuration` [S]**
  ...
  **Update 2026-09-07 — the "safety net" wasn't one.** After `INGEST_INLINE_EXTRACTION` was flipped to `false`, two uploaded PDFs never extracted, and the user reported `cron-tick.yml` failing with `curl (28) Operation timed out after 55000ms`. Root cause: `/api/jobs/tick` runs the same extraction handler under the same ~10s Hobby kill, so the "retry safety net" this item leaned on could never finish an extraction either — the moment inline was off, extraction had **zero** working execution paths (`skill-observations/log.md` Observation 70). **Fix landed:** the drain now runs as a GitHub Actions *job* (`.github/workflows/worker.yml`) executing `worker/index.ts` directly — no Vercel function, no 10s wall. `worker/index.ts` gained a bounded-run mode (`WORKER_EXIT_WHEN_IDLE` / `WORKER_MAX_RUNTIME_MS`); a new `lib/jobs/trigger-worker.ts` fires a `repository_dispatch` from the upload route and the board-pack action so a run starts within seconds of the work being queued; `cron-tick.yml` is deleted (superseded); `/api/jobs/tick` stays as a manual fallback. One-time GitHub setup (secrets + the dispatch PAT) and the Actions-minutes budget are in **`docs/job-worker-github-actions.md`**. `INGEST_INLINE_EXTRACTION=false` is now correct rather than a regression — but the two already-stuck documents need a manual worker run (or re-upload) once the workflow is live.
...
- [x] **4.1 — Move candidate matching to a server-side RPC [L]**
  ...
  **Not verified against a live query plan** at the time this was written — no Supabase connection was available in that session. `npx tsc --noEmit` and `next lint` both pass clean on every changed file. **Update 2026-09-05:** this migration (`20260905000001_match_candidate_entries.sql`) is now applied to the live DB (confirmed via `npx supabase migration list`) — still no live `EXPLAIN`/timing pass recorded, same open sub-item as 1.6.
...
- [x] **4.4 — Add the missing row caps [S]**
  ...
  **Not applied to the live DB at the time this was written** — no Supabase connection was available in that session. `npx tsc --noEmit` passes for all touched files (one unrelated pre-existing error remains in `app/(app)/review/page.tsx:681`, from the concurrent session's Phase 4.1 `match_candidate_entries` work, not touched by this item). `next lint` is clean (one pre-existing unrelated warning in `app/api/jobs/tick/route.ts`). **Update 2026-09-05:** `20260905000002_tax_credit_exposure_bound.sql` is now applied to the live DB (confirmed via `npx supabase migration list`) — including the rolling 2-year `v_tax_credit_exposure` bound, which per this item's own note above was never separately confirmed with the user as a product decision (it mirrors 1.3's precedent rather than a fresh confirmation). Worth flagging to the user once, not re-litigating on every re-audit.
...
- [x] **7.11 — Drop the redundant index [S]**
  ...
  Landed as a new migration (`supabase/migrations/20260905000003_drop_redundant_document_extraction_document_idx.sql`), per this repo's "new migration, don't edit an applied one" convention. First drafted with `DROP INDEX CONCURRENTLY IF EXISTS`, per the same CONCURRENTLY caveat as the other index-adding migrations in this plan — but `npx supabase db push` genuinely failed on it (`SQLSTATE 25001`, CONCURRENTLY cannot run inside a transaction block). Fixed by dropping `CONCURRENTLY` entirely: unlike `CREATE INDEX`, a plain `DROP INDEX` only removes a catalog entry — a brief `ACCESS EXCLUSIVE` lock regardless of table size, not a table-scanning build — so it doesn't need the non-blocking guarantee CONCURRENTLY exists for.
  **Lesson for future migrations in this repo, confirmed by direct observation:** every prior `CREATE INDEX CONCURRENTLY` in this plan (1.1, 1.5, 4.1's `entries_active_amount_idx`) applied successfully via the same `npx supabase db push`, while this `DROP INDEX CONCURRENTLY` failed on the identical transaction-wrapping error — so the CLI's migration runner evidently special-cases `CREATE INDEX CONCURRENTLY` to run outside its transaction wrapper but does not extend the same handling to `DROP INDEX CONCURRENTLY`. Don't assume a header-comment CONCURRENTLY caveat is self-enforcing either way — verify which form the runner actually supports before drafting.
  **Applied to the live DB 2026-09-05.** Confirmed by direct query (`npx supabase db query --linked`, not just migration-history bookkeeping): all of 1.1/1.5/4.1's indexes (`recon_exception_open_extraction_idx`, `recon_exception_open_entry_idx`, `doc_line_item_parent_order_idx`, `entries_active_date_idx`, `source_document_matched_idx`, `vendor_address_normalized_idx`, `document_extraction_unverified_idx`, `entries_active_amount_idx`, `entries_invoice_number_normalized_idx`, `entries_vendor_raw_trgm_idx`) exist on `pg_indexes`, and `document_extraction_document_idx` (this item's target) does not — the drop took effect. This item is fully closed, migration included.
