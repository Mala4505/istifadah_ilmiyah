...
import type { AdminHeadAccountabilityRow } from '@/lib/reports/surfaces/admin-head'
...
// to a person." Ranked by spend. Budget adherence dropped 2026-09-13 when
// admin_head lost its department_id (20260913000001_admin_head_zone_drop_
// department.sql) — every head was seeded under department_id=1 'Venue
// Setup', so the "owning department's budget position" this report used to
// show as adjacent context was never a meaningful attribution. This surface
...
export function AdminHeadAccountabilitySection({
  ...
  return (
    <ReportSection
      ...
      description="Spend, entry volume and document coverage per named administrative head, ranked — the one dimension that attaches a number to a person."
      action={
        <ExportCsvButton
