...
export default async function EntryDetailPage({
  ...
  const [
    ...
    entry.settles_entry_id
      ? supabase
          .from('v_entry_enriched')
          .select('id, ubbl_number, vendor_display_name, vendor_raw, tenant_amount, date')
          .eq('id', entry.settles_entry_id)
          .maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ])

  const headOptions = (headsResult.data ?? []) as HeadOption[]
  ...
  const headById = new Map(headOptions.map((h) => [h.id, h]))
  ...
  function resolveLookup(field: string, value: number): string | null {
    if (field === 'head_id') return headById.get(value)?.name ?? null
      ...
      <Tabs defaultValue="enrichment">
        ...
        <TabsContent value="enrichment" className="flex flex-col gap-4">
          <EnrichmentForm
            ...
            initialHeadId={entry.head_id}
            initialZoneId={entry.zone_id}
            initialHubReference={entry.hub_reference}
            initialEnrichmentNote={entry.enrichment_note}
