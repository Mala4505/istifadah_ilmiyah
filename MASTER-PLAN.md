...
The two Hub-owned statuses are set here and pushed back to Departmental/Audit. This is the one place data flows outward, and it is the reason "push back" is no longer a future-tense pillar. **DESCOPED 2026-08-13:** push-back stays exactly this shape permanently — a generated `.xlsx` batch someone applies by hand on the other side. `format = 'api'` in the constraint below is dead weight, never wired to anything; `'xlsx'` is the only value this system will ever write. Left in the constraint rather than dropped, since removing an unused enum value is pure schema churn with no functional upside.
...
**Acknowledgement is tracked but not required.** `status` moves `generated → delivered → acknowledged` manually, because delivery is a person sending a file — permanently, per the 2026-08-13 decision below, not just for week 1.

**File-based, permanently — DESCOPED 2026-08-13, your decision: no API, ever.** `format = 'api'` stays in the CHECK constraint (harmless, unused) but nothing will ever write it. The generator remains an isolated `entry-fields → module-shape` transform regardless — that structure earns its keep on its own (it's the mirror of the import transform, and keeps the mapping logic in one tested place), not because an API version might reuse it.
...
| Push-back **as a live API call** | The two Hub-owned statuses **ship as a reviewed `.xlsx` export** (§3.7) — that is the whole outward path, working end to end, and it stays that way. **DESCOPED 2026-08-13 (your decision): no API push-back will be built.** | Not planned |
...
1. **Audit-side import** — the mapping is written in Phase 1 against Sheet 2's contract; this wires and tests it against a real file, and populates `audit_status_id`, closing the variance report. **Confirmed daily workflow (§17.23):** export the Departmental portal to `.xlsx` and import it via `/import` first, *then* log into the Audit portal and scrape it with the bookmarklet — the Audit side links to a Hub entry via `main_number`, which only exists once the Departmental import has written it.
2. ~~**API push-back**~~ — **DESCOPED 2026-08-13, your decision: no API, ever.** Status changes flow back the same way data flows in — a generated `.xlsx` (already built, §3.7/§5) applied by hand on the Departmental/Audit side. The two Hub-owned statuses this covers remain "Awaiting Verification" and "Submit to Approval" (§4); nothing else about the export changes. This item is complete as originally scoped and needs no further code.
...
23. **Scraping the Audit portal instead of a manual Excel export — CONFIRMED NEEDED 2026-08-12, architecture decided 2026-08-12.** You asked which is better: (a) you log into the Audit portal yourself in a browser tab, and something reads the data from that already-logged-in tab directly, or (b) you hand the Hub your Audit login and it logs in itself, unattended. **(a) is better, clearly, and here's why:**
   ...
   **UBBL/linkage question RESOLVED 2026-08-13 (corrects an earlier wrong answer):** the Audit portal does **not** expose the UBBL number — it only shows its own Audit entry number. Linking still works, because that entry number is verbatim what the Departmental `.xlsx` export writes into `entries.main_number`; verified **8/8** against the real Departmental fixture and the real Audit screenshot (`test/unit/portal-linkage.test.ts`). The practical consequence is an ordering rule, not a matching-algorithm problem: **the Departmental export must always be imported before the Audit portal is scraped**, so the `main_number` an Audit row needs to match against already exists. **Confirmed daily operator workflow:** export the Departmental portal to `.xlsx` and import it via `/import` → log into the Audit portal and scrape it with the bookmarklet from `/import/bookmarklet` → then work the rest of the dashboard.
...
### Phase 3 — Two-way integration (in progress since 2026-08-12)

**Mine** (once the Audit-side data pipeline is in place)
- [x] **Bookmarklet-based portal table reader — BUILT 2026-08-12 (§17.23).** You log into the portal yourself as normal; a bookmarklet reads the open table from your already-authenticated tab and sends it to the Hub — no credentials ever stored server-side. `public/bookmarklet/read-portal.js`, installed from `/import/bookmarklet`. Reads a jQuery DataTables grid via its own API (so pagination cannot silently truncate the import) and falls back to a plain DOM read; falls back again to a `.json` download if the portal's CSP blocks the upload. **Not yet run against the live portal** — see "Yours" below.
- [x] **Audit-side import — BUILT 2026-08-12.** `lib/import/run-portal-import.ts`. Links an Audit row to a Hub entry on `main_number` (exact only — no fuzzy matching). **Corrected 2026-08-13:** an earlier note here said the Audit portal exposes both numbers — wrong. **The Audit portal does not expose the UBBL number at all**; it only shows its own Audit entry number, which is verbatim what the Departmental `.xlsx` export writes into `entries.main_number`. That linkage is now **verified 8/8 against the real Departmental fixture and the real Audit screenshot** (`test/unit/portal-linkage.test.ts`), pinned so a future renumbering on either side fails the test instead of silently mismatching. The code still tries `ubbl_number` first — harmless, since every Audit row's `ubbl_number` is `null` — before falling through to `main_number`, which is where every real match happens. **Practical consequence: the Departmental `.xlsx` must always be imported before the Audit portal is scraped**, so the `main_number` an Audit row needs already exists (`run-portal-import.ts`'s `audit_row_unmatched` exception message says this explicitly). Writes `audit_status_id` / `audit_status_raw` / `audit_synced_at`, and raises `audit_row_unmatched` / `audit_ambiguous_match` / `tenant_vs_main_variance` rather than guessing. Never creates an entry from the Audit side. Same dry-run-then-commit semantics as the .xlsx path.
- [x] **Departmental-side scrape — BUILT 2026-08-12.** Same bookmarklet, same endpoint; upserts entries on `ubbl_number` to keep identity/money/status current between .xlsx imports. Deliberately does not write `budget_head_id`, `department_id` or `budget_allocation` — the portal's "Budget Head" column names a different level of the hierarchy than the export's (see `lib/import/portal-mapping.ts`), so the .xlsx import remains the source of truth for classification and allocation.
- [ ] **Live-verify both directions against the real portals** — every parser is unit-tested against the Audit screenshot's exact column set and values (43 tests, `test/unit/portal-mapping.test.ts`), and the Audit-to-Hub linkage itself is separately pinned 8/8 against real fixtures (`test/unit/portal-linkage.test.ts`, see above) — but no row has yet made the round trip from a real portal page into `entries`. **Confirmed order for the live run (§17.23): Departmental `.xlsx` import first, Audit portal scrape second** — reversing it will produce `audit_row_unmatched` exceptions, not a hang or silent failure.
...
- [x] ~~API push-back endpoint~~ — **DESCOPED 2026-08-13 (your decision).** Status push-back stays manual `.xlsx` export/import permanently, same as every other direction in this system. Nothing to build; removed from scope, not merely deferred.
...
**Yours**
- [x] **DONE 2026-08-12** — Audit table screenshot supplied; its exact header row and cell values are now the fixture in `test/unit/portal-mapping.test.ts`.
- [ ] Run the confirmed daily workflow once for real: export + import the Departmental `.xlsx` via `/import`, **then** log into the Audit portal and install/run the bookmarklet from `/import/bookmarklet` against it, then send me what the overlay reports (especially any "Unrecognised portal column" warning).
- [x] **RESOLVED 2026-08-13 — the Audit portal does not show the originating UBBL number anywhere**, not hidden and not on the View detail; it only shows its own Audit entry number. Confirmed against the real screenshot and pinned in `test/unit/portal-linkage.test.ts`, so this is no longer open. The mapper still reads a `UBBL Number` column defensively if one is ever present, but the linkage the import actually relies on is `main_number`, not this.
- [ ] Send a screenshot of the verification-stage statuses once they exist (§17.23) — **confirmed 2026-08-13 these are a simple ordered sequence (verification state 1, 2, 3), not a compound/branching model**, so this is a smaller ask than it might read: just the label text for each of the three states. The column is already mapped (`verificationStatus`, via header synonyms) and unseen codes are recorded into `entry_status` for visibility, but nothing is written onto `entries` yet — there's no column for it, and adding one before this confirmation would have been guessing.
