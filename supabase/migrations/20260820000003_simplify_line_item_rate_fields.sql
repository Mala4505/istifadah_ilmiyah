-- Simplifies document_extraction_line_item's rate/amount fields from the
-- original four-way list_rate/discount_pct/net_rate/line_amount split down to
-- rate/discount/amount (20260808000022 for the original DDL).
--
-- Reason: real invoices overwhelmingly print exactly ONE rate per line, never
-- a separate pre-discount "list" price and a discount percentage. Checked
-- against production data (source_document 14/15, vendor Juzer Saleh): every
-- non-null list_rate_ocr exactly equalled net_rate_ocr exactly equalled
-- line_amount_ocr, on every single line item, every time -- the model had no
-- real signal to tell the three apart, so it just copied the one number it
-- could read into all three fields. Three columns carrying one real number
-- is not extra precision, it is three places for that number to drift out of
-- sync on a later manual correction -- exactly the "repetitive"/"copying"
-- symptom reported against the review screen. discount_pct_ocr/verified were
-- always null in the same sample -- there was never a real numeric discount
-- signal being captured, so it is dropped rather than kept alongside
-- discount, which becomes the one place for a discount an invoice actually
-- states in text.
alter table public.document_extraction_line_item
  drop column list_rate_ocr,
  drop column list_rate_verified,
  drop column discount_pct_ocr,
  drop column discount_pct_verified;

alter table public.document_extraction_line_item
  rename column net_rate_ocr to rate_ocr;
alter table public.document_extraction_line_item
  rename column net_rate_verified to rate_verified;
alter table public.document_extraction_line_item
  rename column discount_note_ocr to discount_ocr;
alter table public.document_extraction_line_item
  rename column discount_note_verified to discount_verified;
alter table public.document_extraction_line_item
  rename column line_amount_ocr to amount_ocr;
alter table public.document_extraction_line_item
  rename column line_amount_verified to amount_verified;

-- private.verify_document_extraction / public.verify_document_extraction
-- (20260817000003): same p_document_extraction_id/p_header/p_line_items/
-- p_vendor_id signature, only the line-item JSON keys the function body
-- reads change (list_rate/discount_pct/net_rate/discount_note/line_amount ->
-- rate/discount/amount) plus the column names it writes. No parameter is
-- renamed, so `create or replace` is enough -- no drop needed this time.
...
comment on function private.verify_document_extraction(bigint, jsonb, jsonb, bigint) is
  'Atomic Day-4 review-screen save (MASTER-PLAN §7, §11.2; Phase 2 plan.md §3): writes document_extraction/_line_item _verified columns and inserts rate_reference rows in one transaction. Looks the row up by document_extraction_id directly (multi-bill safe, 20260817000002). Line items use rate/discount/amount, not the earlier list_rate/discount_pct/net_rate/line_amount split (20260820000003). Re-checks reviewer/admin + department visibility itself -- see file header.';
