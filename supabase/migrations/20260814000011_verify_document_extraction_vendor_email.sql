-- Extends private.verify_document_extraction (20260813000002) to also write
-- vendor_email_verified, mirroring vendor_phone_verified exactly. Same
-- signature, same security-definer/permission-check body -- only the update
-- block gains one more assignment and p_header's documented shape gains one
-- more key (the public.verify_document_extraction wrapper's signature is
-- untouched: it always just forwarded p_header as opaque jsonb).
...
comment on function private.verify_document_extraction(bigint, jsonb, jsonb, bigint) is
  'Atomic Day-4 review-screen save (MASTER-PLAN §7, §11.2): writes document_extraction/_line_item _verified columns (including vendor_email_verified, 20260814000011) and inserts rate_reference rows in one transaction. Re-checks reviewer/admin + department visibility itself -- see file header.';

-- public.verify_document_extraction wrapper is unchanged (same signature,
-- p_header just carries one more key) -- see 20260813000002 for its
